...
//
//  Created by Lenin S on 08/07/19.
//  Copyright © 2019 Lenin. All rights reserved.
//

import UIKit

class TIP_VC: UIViewController, UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
    ...
    func configureCellForTableView(_ tableView: UITableView, withCell cell: UITableViewCell, withIndexPath indexPath: IndexPath) {
        ...
        case 1:
            ...
            periodSlider.addTarget(self, action: #selector(TIP_VC.sliderValueDidChange(_:)), for: .valueChanged)
            break;
        case 2:
            let rateOfReturnSlider:UISlider = cell.viewWithTag(332) as! UISlider
            rateOfReturnSlider.addTarget(self, action: #selector(TIP_VC.sliderValueDidChange(_:)), for: .valueChanged)
        default:
            break;
        }
    }
    ...
    @objc func sliderValueDidChange(_ sender: UISlider) {
        ...
        case 222:
            ...
            self.returnPeriodLabel().text = String(format :"%.0f Persons", floor(sender.value)) //"\(floor(sender.value)) years"
            break;
        case 332:
    ...
    func calculateNetReturn(_ amount: Float, period: NSInteger, rateOfReturn: Float) {
        ...
        let tip = amount * rateOfReturn / 100
        let total = amount + tip
        let split = total / Float(period)
        
        self.netReturnLable.text = "\(total)"
        self.actualAmountLable.text = "\(split)"
        self.rolledOverLable.text = "\(tip)"
